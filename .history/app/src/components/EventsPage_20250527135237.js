...
// Helper: Send notification to business when user RSVPs
async function sendGoingNotification(event, user) {
  if (!event.businessId || !user) return;
  try {
    await addDoc(collection(db, "users", event.businessId, "notifications"), {
      type: "going",
      userId: user.uid,
      eventId: event.id,
...
export default function EventsPage() {
  ...
  function handleEventSearch() {
    setEvents((old) => [...old]); // Triggers a re-render
  }
