...
import categoriesData from "../data/categories";  // Your categories list
...
export default function ProfileBuilder() {
  ...
  // Flatten categories list (including subcategories)
  const allCategories = categoriesData.flatMap(
    (cat) => [cat.name, ...cat.subcategories]
  );

  // Show suggestions on input change or focus
  const onBusinessCategoryChange = (e) => {
    const val = e.target.value;
    setForm((prev) => ({ ...prev, businessCategory: val }));

    if (val.trim() === "") {
      // Show all categories if input empty and focused
  ...
  useEffect(() => {
    const handleClickOutside = (event) => {
      if (!event.target.closest(".category-suggestions")) {
        setShowSuggestions(false);
      }
    };
        ...
        {role === "client" && (
          <>
            ...
            <label style={{ display: "block", marginBottom: 12 }}>
              ...
              <Autocomplete
                onLoad={onLoadClient}
                onPlaceChanged={onPlaceChangedClient}
              >
                <input
                  name="location"
                  type="text"
          ...
          <>
            ...
            <label style={{ display: "block", marginBottom: 12, position: "relative" }}>
              ...
              <input
                ...
                onFocus={() => {
                  if (!form.businessCategory) setCategorySuggestions(allCategories);
                  setShowSuggestions(true);
                }}
                autoComplete="off"
                required
