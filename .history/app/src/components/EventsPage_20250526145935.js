  ...
  useEffect(() => {
    async function fetchBusinessesAndEvents() {
      ...
      try {
        ...
        // Fetch events filtered by category if set
        let eventsQuery = query(collection(db, "events"), orderBy("timestamp", "desc"));
    ...
    // Run whenever filterCategory changes
  }, [filterCategory]);
