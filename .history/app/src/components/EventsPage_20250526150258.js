...
const categoriesList = [
  ...
  "Κατηγορία 2",
  "Κατηγορία 3",
  // Βάλε εδώ τις πραγματικές κατηγορίες επιχειρήσεων
];
...
export default function EventsPage() {
  ...
  useEffect(() => {
    async function fetchEventsAndBusinesses() {
      setLoading(true);
      try {
        // Fetch all events (θα φιλτράρουμε μετά τοπικά)
        ...
        if (filterCategory) {
          data = data.filter((evt) => {
            const bizCat = businesses[evt.businessId]?.businessCategory;
        ...
        // Φόρτωσε επιχειρήσεις που υπάρχουν στα events
