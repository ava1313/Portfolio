...
function MapComponent({ businesses, center, onLoad, map, userPosition, navigate }) {
  ...
  useEffect(() => {
    ...
    // Create markers for businesses with simple custom info windows on click
    businesses.forEach((biz) => {
      ...
      geocoder.geocode({ address: biz.businessLocation }, (results, status) => {
        if (status === "OK" && results[0]) {
          ...
          // Use a simple custom icon (you can replace with your own)
          ...
          infoContent.style.color = "#191919";
          infoContent.style.cursor = "pointer";
          infoContent.style.userSelect = "none";
          infoContent.textContent = biz.businessName;

          infoContent.onclick = () => navigate(`/business/${biz.id}`);
