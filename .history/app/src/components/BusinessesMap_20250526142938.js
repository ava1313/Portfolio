...
function MapComponent({ businesses, center, onLoad, map, userPosition, navigate }) {
  ...
  useEffect(() => {
    ...
    const getStarsHTML = (rating) => {
      ...
      if(halfStar) html += emptyStarSVG; // could swap with half star svg if you want
    ...
    businesses.forEach((biz) => {
      ...
      geocoder.geocode({ address: biz.businessLocation }, (results, status) => {
        if (status === "OK" && results[0]) {
          ...
          // Fetch live rating from Places API using FindPlaceFromQuery
          const request = {
            query: biz.businessLocation,
            fields: ["rating", "place_id"],
          };

          placesService.findPlaceFromQuery(request, (results, placesStatus) => {
            let rating = null;
            if (placesStatus === window.google.maps.places.PlacesServiceStatus.OK && results && results.length > 0) {
