...
export default function ResultsPage() {
  ...
  const businessType = query.get("keyword") || ""; // businessType filter
  ...
  useEffect(() => {
    async function fetchBusinesses() {
      ...
      snapshot.forEach((doc) => {
        ...
        // Business type matching (keyword query param)
        const typeMatch =
          !businessType ||
    ...
    fetchBusinesses();
  }, [category, location, businessType]);
...
>
  <img
    ...
    style={{
      height: 68,
      display: "block",
      // filter: "drop-shadow(0 2px 14px #e0e0e0)"  <-- removed this line
    }}
  />
</div>
      ...
      >
        ...
        <span>
          <b>Τύπος Επιχείρησης:</b> {businessType || <i>Καμία</i>}
        </span>
      </div>
