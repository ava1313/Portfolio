...
function MapComponent({ businesses, center, onLoad, map, userPosition, navigate }) {
  ...
  useEffect(() => {
    ...
    businesses.forEach((biz) => {
      ...
      geocoder.geocode({ address: biz.businessLocation }, (results, geocodeStatus) => {
        if (geocodeStatus === "OK" && results[0]) {
          const position = results[0].geometry.location;

          // Debug: log geocode results
          console.log("Geocoded location for", biz.businessName, results[0].formatted_address);

          // Function to create marker and info window once rating is fetched
          const createMarker = (rating) => {
          ...
          // If you have placeId saved, get details by placeId for best accuracy
          if (biz.placeId) {
            ...
            // Otherwise, fallback to textSearch by address for rating
            placesService.textSearch({ query: biz.businessLocation }, (results, textStatus) => {
              let rating = null;
              if (
          ...
          console.warn("Geocode failed for", biz.businessName, geocodeStatus);
        }
      });
    });
