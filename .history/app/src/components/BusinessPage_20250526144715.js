...
export default function BusinessPage() {
  ...
  // Show modal alert (replace window.alert)
  const showModal = (msg) => setModal({ visible: true, message: msg });
  const closeModal = () => setModal({ visible: false, message: "" });

  // Helper: Render stars (full, half, empty) for rating with SVG
  ...
  const averageRating =
    reviews.length === 0 ? null : reviews.reduce((acc, r) => acc + r.rating, 0) / reviews.length;
  ...
  if (loading)
    return (
      <>
        ...
        {modal.visible && <Modal message={modal.message} onClose={closeModal} />}
      </>
    );
