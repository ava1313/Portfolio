...
googleMapsApiKey: "AIzaSyCRtwSDCjGfyulFxOGi4cKR3IQf3i-MTHU",
  ...
  const onPlaceChanged = (fieldName) => {
    if (autocomplete !== null) {
      const place = autocomplete.getPlace();
      if (place.formatted_address) {
        setForm((prev) => ({ ...prev, [fieldName]: place.formatted_address }));
      } else if (place.name) {
        setForm((prev) => ({ ...prev, [fieldName]: place.name }));
      }
    }
  };
          ...
          <>
            ...
            <label style={{ display: "block", marginBottom: 12 }}>
              ...
              {isLoaded ? (
                <Autocomplete
                  onLoad={onLoad}
                  onPlaceChanged={() => onPlaceChanged("businessLocation")}
                >
                  <input
