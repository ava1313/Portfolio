...
export default function BusinessPage() {
  ...
  // Render stars with full and half star logic
  ...
  // Fetch business info, photos, and google rating
  useEffect(() => {
    async function fetchBusiness() {
      ...
      try {
        ...
        if (data.profile?.placeId && window.google?.maps?.places) {
          const service = new window.google.maps.places.PlacesService(document.createElement("div"));
          service.getDetails(
            { placeId: data.profile.placeId, fields: ["rating"] },
            (place, status) => {
              if (status === window.google.maps.places.PlacesServiceStatus.OK && place?.rating) {
