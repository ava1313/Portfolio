...
export default function BusinessPage() {
  ...
  useEffect(() => {
    ...
    const fetchEvents = async () => {
      const eventsCol = collection(db, "users", id, "events");
  ...
  useEffect(() => {
    ...
    const fetchOffers = async () => {
      const offersCol = collection(db, "users", id, "offers");
      const q = query(offersCol, orderBy("timestamp", "desc"));
      const querySnapshot = await getDocs(q);
      const fetchedOffers = [];
      querySnapshot.forEach((doc) => {
